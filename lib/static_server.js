...
const innerWebpackConfig = require('./webpack.config.dev.js');
const webpackMiddleware = require('koa-webpack-dev-middleware');

module.exports = options => {
  const app = new koa();
  app.use(async function(ctx, next) {
    if (!/\.js|\.css/.test(this.url)) {
      return;
    }
    await next();
  });
  ...
  const compiler = webpack(Object.assign(innerWebpackConfig, customWebpackConfig));
