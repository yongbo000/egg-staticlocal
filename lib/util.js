...
function getSubAppEntry(rootDir) {
  ...
  return Array.from(subApps.values()).reduce((entry, item) => {
    const files = globby.sync([ '**/entry/*.{js,less,css}' ], { cwd: item.baseDir });
    for (const f of files) {
      entry[item.name + '_' + f.replace(/\.js$/, '').replace(new RegExp(path.sep, 'g'), '_')] = [ path.join(item.baseDir, f), hmrEntry ];
    }
    return entry;
  }, {});
}
...
function getEntry(rootDir) {
  ...
  const entry = {};
  const files = globby.sync([ '**/entry/*.{js,less,css}' ], { cwd: path.join(rootDir, 'app') });
  for (const f of files) {
    entry[f.replace(/\.js$/, '').replace(new RegExp(path.sep, 'g'), '_')] = [ path.join(rootDir, 'app', f), hmrEntry ];
  }
  return entry;
}
